        ...
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" required class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Product Information -->
                <div class="space-y-6">
                    <h2 class="text-xl font-semibold text-gray-800 pb-2 border-b">Product Information</h2>
                    
                    <!-- Product Parent (Formerly Subcategory) -->
                    <div>
                        <label for="subcategory_id" class="block text-sm font-medium text-gray-700 mb-1">Product*</label>
                        ...
                        @if($subcategories->isEmpty())
                            <p class="text-yellow-600 text-sm mt-1">No products found. Please create a product first.</p>
                        @endif
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1 rounded-md shadow-sm">
                            ...
                            <input type="number" name="price" id="price" min="0" step="0.01" value="{{ old('price', $product->price) }}" class="w-full pl-7 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                        </div>
                        @error('price')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="stock_quantity" id="stock_quantity" min="0" step="1" value="{{ old('stock_quantity', $product->stock_quantity) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" required>
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="checkbox" name="status" id="status" value="1" {{ old('status', $product->status) ? 'checked' : '' }} class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded">
                        <label for="status" class="ml-2 block text-sm text-gray-700">Product is active</label>
                    </div>
                </div>
            </div>
            ...
            <!-- Product Image -->
            <div class="mt-6">
                <h2 class="text-xl font-semibold text-gray-800 pb-2 border-b mb-4">Product Image</h2>
                
                <div class="flex items-center space-x-6">
                    <div class="w-40 h-40 border-2 border-gray-300 border-dashed rounded-lg flex items-center justify-center">
                        <img id="preview" src="{{ $product->image_path ? asset('storage/' . $product->image_path) : '#' }}" alt="Preview" class="{{ $product->image_path ? '' : 'hidden' }} h-full w-full object-cover">
                        <div id="placeholder" class="{{ $product->image_path ? 'hidden' : '' }} text-center p-4">
                    ...
                    <div class="flex-1">
                        ...
                        @if($product->image_path)
                            <p class="mt-1 text-sm text-blue-600">Current image will be kept if no new image is uploaded</p>
                        @endif
                        @error('image')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
